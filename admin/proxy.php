...
function leafext_proxy_help_text() {
	...
	if (!(is_singular()|| is_archive())) {
		...
		$text = $text.'<li>'.sprintf(__('The url to the gpx file is valid only once %s, when elevation is called.','extensions-leaflet-map'),
		'(<a href="https://github.com/wahabmirjan/wp-simple-nonce">wp-simple-nonce</a>)').'</li>';
...
[testelevation proxy=1 gpx="https://your-domain.tld/path/to/track.gpx" option=...]</code></pre>';
	...
	if (!(is_singular()|| is_archive())) {
		...
		$text = $text.'<p>'.
			...
			sprintf(__("To add a directory enter the url in text field. It must begin with %s, but does not need to be the same host as in the example.",
			"extensions-leaflet-map"),'<i>https://</i>').' '.
