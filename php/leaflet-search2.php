...
function leafext_leafletsearch_function($atts,$content,$shortcode) {
    ...
    if (strpos($options['propertyName'],',') !== false) {
      $properties=explode(',',$options['propertyName']);
      $options['propertyName'] = "title";
      //unset($options['propertyName']);
      $jsoptions = $options;
      unset($jsoptions['propertyName']);
      return leafext_leafletsearch2_script($options,trim(preg_replace('/\s+/', ' ',leafext_java_params($jsoptions))),$properties);
    }
...
function leafext_leafletsearch2_script($options,$jsoptions,$properties){
  $text = '
  <script>
  window.WPLeafletMapPlugin = window.WPLeafletMapPlugin || [];
  window.WPLeafletMapPlugin.push(function () {
    let att_property = '.json_encode($options['propertyName']).';
    let att_zoom = '.json_encode($options['zoom']).';
    let all_properties = '.json_encode($properties).';
    ';
    ob_start();
    include (TESTLEAFEXT_PLUGIN_DIR.'/js/leaflet-search-geojsonPopup.js');
    $text = $text.ob_get_clean();
		$text = $text.'
  });
  </script>';
