...
function testleafext_cluster_function( $atts ){
	...
	wp_enqueue_script('my_markercluster',
		plugins_url('js/markercluster.js',TESTLEAFEXT_PLUGIN_FILE),
		array('wp_leaflet_map'), null);
	...
	$cluster_options = shortcode_atts( array('zoom' => $defaults['zoom'], 'radius' => $defaults['radius'], 'spiderfy' => $defaults['spiderfy']), $atts);
...
add_shortcode('testcluster', 'testleafext_cluster_function' );
?>
